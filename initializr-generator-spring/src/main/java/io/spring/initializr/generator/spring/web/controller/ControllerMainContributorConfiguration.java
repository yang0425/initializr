...
public class ControllerMainContributorConfiguration {
	...
	public MainCodeContributor<ControllerMainCustomizer> controllerMainCodeContributor(
		...
		return new MainCodeContributor<>(this.packageName, CLASS_NAME, ControllerMainCustomizer.class, codeCustomizers);
	}
	...
	public ControllerMainCustomizer controllerMainFieldCustomizer() {
		return (typeDeclaration) -> typeDeclaration.addFieldDeclaration(JavaFieldDeclaration.field(SERVICE_FIELD_NAME)
				.modifiers(Modifier.PRIVATE | Modifier.FINAL).returning(this.serviceClass));
	}
	...
	public ControllerMainCustomizer controllerMainConstructorCustomizer() {
		return (typeDeclaration) -> typeDeclaration.addConstructorDeclaration(JavaConstructorDeclaration.constructor()
				.modifiers(Modifier.PUBLIC).parameters(new Parameter(this.serviceClass, SERVICE_FIELD_NAME))
				.body(new JavaExpressionStatement(new JavaStringExpression(
						String.format("this.%s = %s", SERVICE_FIELD_NAME, SERVICE_FIELD_NAME)))));
	}
	...
	public ControllerMainCustomizer controllerMainMethodCustomizer() {
		return (typeDeclaration) -> {
			JavaMethodDeclaration methodDeclaration = JavaMethodDeclaration.method("helloWorld")
					.modifiers(Modifier.PUBLIC).returning(this.dtoClass)
