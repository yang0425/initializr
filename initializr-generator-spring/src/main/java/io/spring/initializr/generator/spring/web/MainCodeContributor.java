...
public class MainCodeContributor<T extends CodeCustomizer> implements
    ...
    public MainCodeContributor(String packageName, String className,
                               Class<T> customizerClass, ObjectProvider<T> codeCustomizers) {
    ...
    public void customize(SourceCode<TypeDeclaration, CompilationUnit<TypeDeclaration>> sourceCode) {
        CompilationUnit<TypeDeclaration> compilationUnit =
            sourceCode.createCompilationUnit(this.packageName, className);
        TypeDeclaration typeDeclaration = compilationUnit.createTypeDeclaration(className);
        List<T> customizers = this.codeCustomizers.orderedStream().collect(Collectors.toList());
        LambdaSafe.callbacks(customizerClass, customizers, typeDeclaration)
